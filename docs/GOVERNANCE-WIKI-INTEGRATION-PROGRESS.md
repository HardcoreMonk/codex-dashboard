...
`codex-dashboard`에 `codex-project-mgmt`의 프로젝트 관리 및 production wiki 운영 기능을 통합하는 1차 MVP를 구현했다.

관리 화면에서 `codex-zone/projects.yaml`, `codex-zone/wiki/`, `codex-zone/raw/`, 최신 audit 리포트 상태를 확인하고, 신규 프로젝트를 registry에 등록한 뒤 `project-sync.sh`, `wiki-check.sh`, `zone-track.sh`를 실행할 수 있다. 실행 이력은 기존 관리자 감사 로그에 기록된다.
...
- 액션:
  ...
  - 프로젝트 동기화
  - Zone 추적
- 감사 로그 필터에 `governance_check`, `governance_sync`, `governance_track`, `governance_project_add`를 추가했다.
